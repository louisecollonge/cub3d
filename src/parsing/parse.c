...
/*   Updated: 2025/03/13 19:51:08 by lcollong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	data_check(t_data *data)
{
	...
	if (!data->map_string)
		error("No map", data);	
	if (wall_outline(data) == false) // wall_outline doit rester avant empty_space 
		error("The outline of the map must be walls", data);
	if (empty_space(data) == false)
		error("The map must not have any empty space within its walls", data);
...
void	parse_file(char *map)
{
	...
	if (data->fd_map < 0)
		error("Map file opening failure", data);
	while (data->fd_map >= 0)
	{
